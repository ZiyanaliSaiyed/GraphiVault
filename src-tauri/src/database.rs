...
pub async fn init_db(pool: &SqlitePool) -> Result<()> {
    ...
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_images_file_hash ON images(file_hash)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_images_created_at ON images(created_at)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_images_updated_at ON images(updated_at)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_images_storage_path ON images(storage_path)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_tags_image_id ON tags(image_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_tags_created_at ON tags(created_at)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_annotations_image_id ON annotations(image_id)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_auth_logs_timestamp ON auth_logs(timestamp)")
        .execute(pool)
        .await?;
    sqlx::query("CREATE INDEX IF NOT EXISTS idx_auth_logs_event_type ON auth_logs(event_type)")
        .execute(pool)
        .await?;
    // Initialize vault metadata if not exists
    let count: (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM vault_meta WHERE key = 'schema_version'")
            .fetch_one(pool)
            .await?;
...
pub async fn insert_annotation(pool: &SqlitePool, annotation: &AnnotationRecord) -> Result<i64> {
    let result =
        sqlx::query("INSERT INTO annotations (image_id, note, created_at) VALUES (?, ?, ?)")
